...
personas.forEach(element => {
  let infoPersona = {
    ...
    colorOjos: element.eyeColor,
  }
  arrayPersonas.push(infoPersona)
});

//Para crear y mostrar a la primera persona
let primeraPersona = document.getElementById('first-person');
let boton = document.getElementById('boton');

    // primeraPersona.innerHTML = `
    // <img src="${arrayPersonas[0].imagen}" class="card-img-top" alt="...">
    //         <div class="card-body">
    //             <h1 class="card-title">${arrayPersonas[0].nombre}</h1>
    //             <p class="card-text">Edad: ${arrayPersonas[0].edad}</p>
    //             <p class="card-text">Email: ${arrayPersonas[0].email}</p>
    //             <p class="card-text">Direccion: ${arrayPersonas[0].direccion}</p>   
    //             <p class="card-text">Color de ojos: ${arrayPersonas[0].colorOjos}</p>
    //             </div>`

    // boton.addEventListener('click', () => {
    //   primeraPersona.style.backgroundColor = arrayPersonas[0].colorOjos;
    // })


//Mostrar una persona segun el numero que eliga el usuario
    // let numero = Number(prompt("Introduce un numero: "))

    // if (numero > 0 && numero <= arrayPersonas.length) {
  
    //   primeraPersona.innerHTML = `
    //     <img src="${arrayPersonas[numero - 1].imagen}" class="card-img-top" alt="...">
    //     <div class="card-body">
    //         <h1 class="card-title">${arrayPersonas[numero - 1].nombre}</h1>
    //         <p class="card-text">Edad: ${arrayPersonas[numero - 1].edad}</p>
    //         <p class="card-text">Email: ${arrayPersonas[numero - 1].email}</p>
    //         <p class="card-text">Dirección: ${arrayPersonas[numero - 1].direccion}</p>   
    //         <p class="card-text">Color de ojos: ${arrayPersonas[numero - 1].colorOjos}</p>
    //     </div>
    //   `;

    //   boton.addEventListener('click', () => {
    //     primeraPersona.style.backgroundColor = arrayPersonas[numero - 1].colorOjos;
    //   });
    // } else {
    //   alert("No se encuentra a esa persona o no existe en el array de personas");
    //   primeraPersona.innerHTML = `<p class="text-danger">Número fuera de rango</p>`;
    // }


//Muestra la información de todas las personas cuyo nombre contenga el texto escrito
      // let texto = prompt("Introduce un texto para buscar en los nombres: ").toLowerCase();

      // primeraPersona.innerHTML = "";


      // let resultados = arrayPersonas.filter(persona =>
      ...
      //             <p class="card-text">Dirección: ${p.direccion}</p>   
      //             <p class="card-text">Color de ojos: ${p.colorOjos}</p>
      //         </div>
      //       </div>
      //     `;
      //   });
      ...
      //   boton.addEventListener('click', () => {
      //     document.querySelectorAll('#first-person .card').forEach((card, index) => {
      //       card.style.backgroundColor = resultados[index].colorOjos;
      //     });
      //   });

      // } else {
      //   primeraPersona.innerHTML = `<p class="text-danger">No se encontraron personas con ese nombre.</p>`;
      // }
...
boton.addEventListener('click', () => {
  ...
  mostrarPersonas(resultados.length > 0 ? resultados : arrayPersonas);
});